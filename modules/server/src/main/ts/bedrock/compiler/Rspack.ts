...
const getWebPackConfigTs = (tsConfigFile: string, scratchFile: string, dest: string, coverage: string[], manualMode: boolean, basedir: string): RspackOptions => {
    ...
    ? () =>
      Object.fromEntries(
        fs.readdirSync(modulesDir)
          ...
          .map((name) => {
            if (name === 'persona') {
              const tsMain = path.join(modulesDir, name, 'persona/src/main/ts/main.ts ');
              return [`@ephox/${name}`, tsMain];
            }
            const tsMain = path.join(modulesDir, name, 'src/main/ts/ephox', name, 'api/Main.ts');
            return [`@ephox/${name}`, tsMain];
          }))
    : () => ({});
  ...
  return {
    ...
    plugins: [
      ...
      // NOTE: Optional plugins that can be enabled for debugging rspack issues
