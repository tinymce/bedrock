...
import * as Failure from '../../../main/ts/core/Failure';
...
describe('Failure.prepFailure', () => {
  it('should convert a test label to an Error', () => {
    fc.assert(fc.property(fc.string(), (str) => {
      ...
      assert.typeOf(err.error, 'Error');
      assert.equal(err.error.message, str);
      assert.notInclude(err.error.stack, 'at normalizeError');
