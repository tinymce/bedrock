...
- Safari 26 could not load the test page through the LambdaTest tunnel, which hardcoded `localhost`. The tunnel URL now uses LambdaTest's `localhost.lambdatest.com` DNS alias for all browsers, and honours `--pageHost` when one is given. #TINYMCE-14879
...
### Fixed
- Custom scratch folders moved inside scratch dir. #TINY-14050
